...
- **3D render** — Blender renders of the real cassette (`pipetting_vfa`, `screwing_vfa`,
  ...
  case twists.
- **Real footage** — filmed lab demonstrations (`vfa_initial_video`, `vfa_assemble_video`, from
  VFA_App_Real). A real gloved hand doing the real thing.

Two of those renders are generated by `process_vfa.py` (in the Blender project, not this repo)
straight from the reader and cassette STLs, and cover the two actions the older clips never
showed:

| Clip | Shows | Used on |
|---|---|---|
| `reader_mount_3d` | the reader and phone coming together, on a clinic bench | step 1, *Fit the reader* |
| `cassette_insert_3d` | the bottom case sliding under the reader and lifting into the chamber | steps 3 and 14, the two photo steps |

Both are cut across multiple camera angles so the action stays readable, and paced to be
followed in real time rather than skimmed.
